...
Newsworthiness scoring and item summaries are AI-generated by Claude.
...
4. **Score + summarize** each new item with Claude (`scoring.py`). A single API
   ...
   With no `ANTHROPIC_API_KEY` set, the pipeline still runs and leaves these
   fields blank.
...
cp .env.example .env          # add your ANTHROPIC_API_KEY (+ any private feed keys)
...
`scoring.py` sends each item's title and summary to Claude Haiku in a single
call and gets back three things:
...
scale for your beat. Scoring requires an `ANTHROPIC_API_KEY` (see
...
├── scoring.py            # analyze_item(): Claude-based score + summary
...
- `anthropic` — Claude API for scoring + summaries
